...
#include <signal.h> // REQUIRED for signal() and SIGINT handler
...
// WARNING: THESE GLOBAL VARIABLES ARE NOT THREAD/CONCURRENCY SAFE!
// In a real multi-client server, access to these MUST be protected by mutexes
// and organized by document ID.
...
struct per_session_data__editor {
    ...
    char username[64]; // New: Username from client
...
/**
 * @brief Frees global memory allocated for collaborative state.
 */
void free_all_globals()
{
    ...
    if (pending_edits) {
        // We only free the array itself, assuming resolve_pending_edits cleaned up line_edit members.
        // If the server shuts down before resolution, memory for line_edit members will leak.
        free(pending_edits);
        pending_edits = NULL;
    }
}
    ...
    switch (reason) {
        ...
        case LWS_CALLBACK_RECEIVE:
        {
            ...
            if (strstr(buffer, "\"type\":\"ACCESS_CODE_CHECK\"")) 
            {
                ...
                char user_id[64] = {0};
                char username[64] = {0}; // New: Capture username
                
                // WARNING: This JSON parsing using sscanf is extremely fragile. 
                // It will break if the JSON keys are reordered or if any data contains escaped characters.
                // In a production environment, you MUST use a robust C JSON library (e.g., Jansson).
                if (sscanf(buffer, "{\"type\":\"ACCESS_CODE_CHECK\",\"code\":\"%11[^\"]\",\"user_id\":\"%63[^\"]\",\"username\":\"%63[^\"]\"}", 
                           code, user_id, username) == 3)
                {
                    ...
                    if (lookup_code(code, &found_user) && strcmp(found_user.perm, "EDIT") == 0)
                    {
                        ...
                        strncpy(pss->username, username, 64); // Store username
                        ...
                        lwsl_notice("Client authenticated for code %s (Hier: %c, UID: %s, Username: %s)\n", 
                                    code, found_user.hierarchy, user_id, username);
            ...
            else if (strstr(buffer, "\"type\":\"SUBMIT_EDIT\"")) 
            {
                ...
                // WARNING: The logic below for line_num and new_text extraction is highly symbolic.
                // Real collaborative editing requires parsing the actual line number and content accurately
                // from the JSON, which is impossible without a robust JSON parser.
                int line_num;
